...
public class Manage {
	...
	public void run() {
		while(true) {
			System.out.println("[학생관리 시스템]\n1.학생등록 2.학생탈퇴 3.로그인 4.전체학생 조회 5.종료");
			int sel = this.stum.sc.nextInt();
			if(sel == 1) {
				this.stum.inrollmentStudent();
			}else if(sel == 2) {
				this.stum.withdrawlStudent();
			}else if(sel == 3) {
				login();
				if(log != -1) {
					while(log != -1) {
						System.out.println("1.로그아웃 2.수강신청 3.수강철회 4.점수 조회");
						int select = this.stum.sc.nextInt();
						if(select == 1) {
							log = -1;
						}else if(select == 2) {
							this.subM.subApplicatin();
						}else if(select == 3) {
							this.subM.subWithdrawal();
						}else if(select == 4) {
							this.subM.print();
						}
					}
	...
	public void login() {
		...
		if(this.stum.check(num)!=-1) {
			log = this.stum.check(num);
		}
	}
}
